...
if (u_name) {
 ...
 呼叫localStorage中的topThree
var topthree_answer = JSON.parse(localStorage.getItem('topThree'));
...
for (var i = 0; i < topthree_answer.length; i++) {
    ...
    // 使用模板字串動態生成變數名稱
    var variablePrefix = 'name_';
    var variableName = variablePrefix + (i + 1); // 產生name_1、name_2
    p_name = window[variableName]; // 設置為相應的產品名稱

    variablePrefix = 'img_';
    variableName = variablePrefix + (i + 1); // 產生img_1、img_2
    img = window[variableName]; // 設置為相應的圖像路徑
}
...
productsInfo.forEach(function (item, index) {
    ...
    var cardHTML = `
        <div class="card mb-4" style="max-width: 70%;">
            <div class="row g-0">
                <div class="col-md-4">
                    <img src="(/css/health_items/img/${item.img}" class="img-fluid rounded-start" alt="...">
                </div>
                <div class="col-md-8">
                    <div class="card-body">
